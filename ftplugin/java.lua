...
local config = {
	...
	on_attach = function(client, bufnr)
		...
		-- ftplugin/java.lua  (add inside the LspAttach autocmd block from Stage 12, alongside <leader>jo etc.)
		vim.keymap.set("n", "<leader>jg", function()
			require("jdtls").generate_getters_setters()
		...
		vim.keymap.set("n", "<leader>jt", function()
			require("jdtls").generate_to_string()
		end, vim.tbl_extend("force", opts, { desc = "Generate toString" }))

		vim.keymap.set("n", "<leader>je", function()
			require("jdtls").generate_equals_hashcode()
		end, vim.tbl_extend("force", opts, { desc = "Generate equals/hashCode" }))
		vim.keymap.set(
			"n",
			"<leader>jv",
	...
	callback = function(args)
		...
		if client and client.name == "jdtls" then
			...
			vim.keymap.set(
				...
				vim.tbl_extend("force", opts, { desc = "Debug: test nearest method" })
			)
		end
	end,
})
